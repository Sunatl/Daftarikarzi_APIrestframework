...
class Wallet(models.Model):
    ...
    def deduct_funds(self, amount):
        if self.balance >= amount:
            ...
            raise ValueError("Маблағи кофӣ дар ҳамён вуҷуд надорад.")

    def __str__(self):
        return f"{self.customer.name} - Баланс: {self.balance} TJS"
...
class Loan(models.Model):
    ...
    def __str__(self):
        return f"{self.customer.name} - {self.total_amount} TJS (давомнокӣ: {self.duration})"
...
class Payment(models.Model):
    ...
    def __str__(self):
        return f"Пардохт {self.amount} TJS барои {self.loan.customer.name}"
